...
onMounted(() => {
  ...
  if (typeof params.exercices === 'string')
    exercices.value = exercicesStringArrayToObjectArray(params.exercices.split(',').filter(e => e !== ''))
})
</script>
      ...
      <div flex="~ col" grow gap-4 pt-14 md:px-12 :style="{ background: chromakeyMode ? chromakeyBgColor : '', color: chromakeyMode ? chromakeyTextColor : '' }">
        ...
        <template v-else>
          ...
          <Draggable
            v-model="exercices"
            item-key="id"
            class="w-max inline-flex flex-col gap-2"
            @start="drag = true"
            @end="drag = false"
          >
            <template #item="{ element, index }">
              <li relative w-max flex cursor-pointer items-center gap-1 p-3 text="xl md:3xl" :class="[exerciceIsRunning(index) && 'text-lime-400', exerciceIsResting(index) && 'text-amber', stopped && 'group']">
